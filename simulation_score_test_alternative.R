...
# Parameters
# TO BE CHANGED
gamma_alt  <- matrix(c(0, 1.5, sqrt(3)/2, 5/sqrt(n), 0, 0, sqrt(3), 5/sqrt(n)),
                     nrow = 2, byrow = T)
gamma_null <- matrix(c(0, 1.5, sqrt(3)/2, 0, 0, 0, sqrt(3), 0),
                     nrow = 2, byrow = T)
...
cl      <- parallel::makeCluster(N_cores - 3)
...
# SAVING RESULTS
###############################################################################
save_name = "simulations_data/simulation_score_test_alternative.RData"
